...
/**
  * Computes indexes to a per-pixel acces of a 2D image, based on threadIdx and blockIdx. <br>
  * Morover, threads in a warp will be arranged in a rectangle (rather than in single line as with the naive implementation). <br>
...
/**
* Transform sampleCount to color in linear grayscale from black (0) to white (sampleCount100Percent)
* @param sampleCount will be trimmed to be lesser or equal to sampleCount100Percent. 
 */
__device__ 
...
uint sampleTheFractal(Pointi pixel, Pointi gridSize, Rectangle<Real> image, uint maxIterations,float & sampleCountF, bool adaptiveSS){
  ...
  //We are in a complex plane from {@code left_bottom} to {@code right_top}, so we scale the pixels to it
  ...
  for(uint i = 0; i < sampleCount; i++){
    ...
    //decide whether to continue with supersampling or not
    if(adaptiveSS && ( i > 0 && i < adaptiveTreshold) || (i == sampleCount / 2) ){

      float mean = escapeTimeSum / (i+1);
      float dispersion = computeDispersion(samples, i, mean);
      ...
      else if(__ALL(dispersion < treshold)){ // uniform distribution - not chaotic
        sampleCount = i+1; //terminating this cycle and storing info about actual number of samples taken
      }
      else if(__ALL(dispersion <= 1) && i >= sampleCount / 2){ //not that much chaotic -- take up to half of max allowed samples
        sampleCount = i+1; //terminating this cycle and storing info about actual number of samples taken
      }
      //else: chaotic
    }
...
/**
 * @param image segment of the complex plane that is to be rendered as an image (region of interest)
 */
 template <class Real> __device__
...
/*
 * For given point <code>p</code> in the current image and given warping information, find cooridnates of the same point (=representing the same  *oint in the fractal's complex plane) in the image being warped
...
/**
 * Returns how many samples this pixel should take, based on foveation. <br>
 * Value is between 0 and maxSuperSampling, resp 0 or 1 for maxSuperSampling < 1. <br>
 * Value in the focus will always be maxSuperSampling for maxSuperSampling >=1. <br>
 * When maxSuperSampling < 1, then focus radius is lowered and pixels within still get result 1. <br>
...
pixel_info_t readFromArrayUsingFiltering(pixel_info_t** textureArr, long textureArrPitch, Point<Real> coordinates){
    ...
    Real xB = coordinates.x; //note that in canonical implementation, here is {@code coordinates.x - 0.5}. With our coordinate-system, we omit this.
...
/**
 *  Render fractal, with additional support for foveated rendering and sample reuse
 */
template <class Real> __device__ 
...
/**
 * Render the fractal with CUDA, using adaptive super-sampling but no other heuristics. <br>
 * Computes in single floating-point precision.
 * @param output CUDA 2D pitched array allocated by the caller to write output to, with dimensions {@code outputSize}
 * @param outputPitch pitch of the output CUDA pitched array.
 * @param outputSize size of the output CUDA pitched array.
 * @param image region of interest, rectangular segment of the plane that is to be renered as fractal
 * @param maxIterations bound of algorithm's complexity
 * @param maxSuperSampling parameter of the adaptive super sampling heuristic: max number of samples per pixel. Value lower than 1 have special interpretation and are allowed.
 * @param flags bit flags, representing parameters of the heuristics or parameters of the kernels. See FLAGS definition in this file for details.
 *
 */
extern "C" __global__
...
/**
 * Render the fractal with CUDA, using adaptive super-sampling but no other heuristics. <br>
 * Computes in double floating-point precision.
 * @param output CUDA 2D pitched array allocated by the caller to write output to, with dimensions {@code outputSize}
 * @param outputPitch pitch of the output CUDA pitched array.
 * @param outputSize size of the output CUDA pitched array.
 * @param image region of interest, rectangular segment of the plane that is to be renered as fractal
...
/**
 * Render the fractal with CUDA, using adaptive super-sampling and the real-time heuristic-based toolset. <br>
 * Computes in single floating-point precision.
 * @param output CUDA 2D pitched array allocated by the caller to write output to, with dimensions {@code outputSize}
 * @param outputPitch pitch of the output CUDA pitched array.
 * @param outputSize size of the output CUDA pitched array.
 ...
 * @param maxSuperSampling parameter of the adaptive super sampling heuristic: max number of samples per pixel. Value lower than 1 have special interpretation and are allowed.
 * @param flags bit flags, representing parameters of the heuristics or parameters of the kernels. See FLAGS definition in this file for details.
 * @param imageReused region of interest of the reused rendering, rectangular segment of the plane that has been rendered to {@code input} array.
 * @param input CUDA 2D pitched array with data to be used for sample-reuse. Usually filled with data by previous calls of a rendering kernel with this array as output.
 * @param inputPitch pitch of the {@code input} CUDA 2D pitched array.
 * @param focus location of the user focus (e.g. cursor), using the texture coordinate system (i.e. (0,0) to (width, height))
 *
 */
extern "C" __global__
...
/** 
 * Transform {@code inputMain} to the final color and write it to {@code surfaceOutput}.
 * @param maxSuperSampling is here only for the purpose of visualizing the sample count
 * @param surfaceOutput cuda surface wrapper of the texture to write the result to
 */
extern "C" __global__
